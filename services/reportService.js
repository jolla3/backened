...
const getMonthlyReport = async (year, month, cooperativeId) => {
  ...
  const porterPipeline = [
    ...
    { $lookup: { from: 'porters', localField: '_id', foreignField: '_id', as: 'porterInfo' } },
  ...
  const porterStats = await Porter.aggregate([
    ...
    { $lookup: {
        ...
        let: { porterId: '$_id' },
        pipeline: [
          { $match: { $expr: { $eq: ['$porter_id', '$$porterId'] }, timestamp_server: { $gte: startDate, $lte: endDate }, type: 'milk' } },
          { $group: { _id: null, totalLitres: { $sum: '$litres' }, totalPayout: { $sum: '$payout' }, count: { $sum: 1 } } }
        ],
        as: 'stats'
      }
    },
  ...
  const dailyPipeline = [
    ...
    { $project: { date: '$_id', litres: 1, payout: 1, transactionCount: 1, _id: 0 } },
    { $sort: { date: 1 } }
  ];
  ...
  const milkChangePercent = prevMonth.totalLitres ? ((totalLitres - prevMonth.totalLitres) / prevMonth.totalLitres) * 100 : 0;
  const payoutChangePercent = prevMonth.totalPayout ? ((totalPayout - prevMonth.totalPayout) / prevMonth.totalPayout) * 100 : 0;
  const txChangePercent = prevMonth.transactionCount ? ((transactionCount - prevMonth.transactionCount) / prevMonth.transactionCount) * 100 : 0;
